defmodule FakeServer.InstanceTest do
  ...
  describe "#run" do
    test "returns {:error, reason} when server_name is not an atom" do
      ...
      assert {:error, {"invalid", "server name must be an atom"}} ==
               Instance.run(server_name: "invalid")
    ...
    test "returns {:error, reason} when max_conn is not a positive integer" do
      assert {:error, {"ten", "max_conn must be a positive integer"}} ==
               Instance.run(max_conn: "ten")

      assert {:error, {10.3, "max_conn must be a positive integer"}} ==
               Instance.run(max_conn: 10.3)
    end
